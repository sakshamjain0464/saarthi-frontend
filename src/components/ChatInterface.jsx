...
export default function ChatInterface({
  ...
  cities = [] // Add cities prop for map data
}) {
  ...
  // Default cities data (replace with dynamic data)
  const mapCities = cities.length > 0 ? cities : [
  ...
  return (
    <div className={`chat-map-container ${isMobile ? 'block' : 'grid'}`}>
      {/* Chat Container */}
          ...
          {iterinary && iterinary.length > 0 && (
            <div className='mb-4 text-left'>
              <div className='inline-block p-3 rounded-lg bg-blue-100 text-blue-900'>
                ...
                >
                  <Volume2 className='w-4 h-4 mr-2' />
                  {speakingMessage === iterinary && isSpeaking ? 'Stop' : 'Speak'}
                </Button>
              </div>
            </div>
          )}
          ...
          {isModifyIterinary && (
            <div className='my-2 mx-auto text-center'>
              ...
              <div className='flex justify-center space-x-4'>
                ...
                <Button onClick={() => setIsModifyIterinary(false)} variant='outline'>
                  No
                </Button>
              </div>
            </div>
        ...
        <div className='p-4 border-t'>
          ...
          {isPostItinerary && (
            <div className='mt-4 flex space-x-2'>
              ...
              <Button onClick={() => setIsEditorOpen(true)} variant='outline' className='flex-grow'>
                Edit Itinerary
              </Button>
            </div>
          )}
      ...
      <div className={`map-container ${isMobile ? 'fixed inset-0 bg-black/50 z-50' : 'h-full'} ${isMobile && !showMapPopup ? 'hidden' : ''}`}>
        <div className={`${isMobile ? 'h-4/5 w-full max-w-2xl mx-auto bg-white rounded-lg' : 'h-full'}`}>
          <MapContainer
